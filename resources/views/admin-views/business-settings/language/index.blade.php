    ...
    <div class="content container-fluid">
        ...
        <div class="mb-4 pb-2">
            <h2 class="h1 mb-0 text-capitalize d-flex align-items-center gap-2">
                <img src="{{asset(main_path().'assets/back-end/img/system-setting.png')}}" alt="">
                            ...
                            @foreach(json_decode($language['value'],true) as $key =>$data)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        @else
                                            <form action="{{ route('business-settings.language.update-status') }}" method="GET" id="language_id_{{$data['id']}}_form" class="language_id_form">
                                                @csrf
                                    ...
                                    <td>
                                        ...
                                        <form action="{{route('business-settings.language.update-default-status', ['code'=>$data['code']])}}" method="GET" id="language_default_id_{{$data['id']}}_form" class="language_default_id_form">
                                            @csrf
                                            <input type="hidden" name="code" value="{{$data['code']}}">
                                            <label class="switcher mx-auto">
                                                ...
                                                @if($data['code']!='en')
                                                    ...
                                                    @else
                                                        <a class="dropdown-item delete"
                                                            id="{{route('business-settings.language.delete',[$data['code']])}}">{{translate('delete')}}</a>

                                                    @endif
                                                @endif
                                                <a class="dropdown-item"
                                                    href="{{route('business-settings.language.translate',[$data['code']])}}">{{translate('translate')}}</a>
                                            </div>
                                        </div>
                                    </td>
                                </tr>
        ...
        <div class="modal fade" id="lang-modal" tabindex="-1" role="dialog"
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <form action="{{route('business-settings.language.add-new')}}" method="post"
                                ...
                                <div class="col-6">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control country-var-select w-100" name="code">
                                            @foreach(\Illuminate\Support\Facades\File::files(base_path(file_main_path().'flags')) as $path)
                                                @if(pathinfo($path)['filename'] !='en')
                                                    <option value="{{ pathinfo($path)['filename'] }}"
                                                            title="{{ asset(main_path().'flags/'.pathinfo($path)['filename'].'.png') }}">
            ...
            <div class="modal fade" id="lang-modal-update-{{$data['code']}}" tabindex="-1" role="dialog"
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        ...
                        <form action="{{route('business-settings.language.update')}}" method="post">
                            @csrf
                            <div class="modal-body">
                                <div class="row">
                                    <div class="col-12">
...
@push('script')
    ...
    <script>
        ...
        $('.language_id_form').on('submit', function(event){
            ...
            $.ajax({
                url: "{{ route('business-settings.language.update-status') }}",
                method: 'GET',
                data: $(this).serialize(),
