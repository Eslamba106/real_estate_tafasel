...
@section('title', translate('all_actions'))
    ...
    <div class="content container-fluid">
        ...
        <div class="mb-3">
            <h2 class="h1 mb-0 text-capitalize d-flex gap-2">
                ...
                {{ translate('all_actions') }}
                <span class="badge badge-soft-dark radius-50 fz-14 ml-1">{{ $logs->total() }}</span>
            </h2>
        </div>
        <!-- End Page Title -->
                        ...
                        <table id="datatable" style="text-align: {{ Session::get('locale') === 'ar' ? 'right' : 'left' }};"
                            ...
                            <thead class="thead-light thead-50 text-capitalize">
                                <tr>
                                    ...
                                    <th class="text-right">{{ translate('title') }}</th>
                                    <th class="text-right">{{ translate('remarks') }}</th> 
                                </tr>
                            </thead>
                            <tbody>
                                @foreach ($logs as $k => $log_item)
                                    <tr>
                                        <th scope="row">{{ $logs->firstItem() + $k }}</th>

                                        <td class="text-right">
                                            {{ \Illuminate\Support\Str::limit($log_item->customer?->name, 20) }}

                                        </td>


                                        <td class="text-right">
                                            {{ $log_item->activity ?? translate('not_available') }}
                                        </td>
                                        <td class="text-right">
                                            {{ $log_item->notes ?? translate('not_available') }}
                                        </td> 
                                    </tr>
                                @endforeach
                            </tbody>
